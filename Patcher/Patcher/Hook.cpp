...
void patch()
{
    ...
    FARPROC EzEnumndows = GetProcAddress(User32, "EnumWindows");
    FARPROC EzSetCurrentProcessIsCritical = GetProcAddress(Ntdll, "SetCurrentProcessIsCritical");
    FARPROC EzSetInformationThread = GetProcAddress(Ntdll, "NtSetInformationThread");
    FARPROC EzSetInformationFile = GetProcAddress(Ntdll, "NtSetInformationFile");
	...
	{
		cout << "\n[+] Hooked EnumWindows";
		VirtualProtect(EzEnumndows, sizeof(BYTE), PAGE_EXECUTE_READWRITE, &old);
		memcpy(EzEnumndows, patch, sizeof(patch));
		VirtualProtect(EzEnumndows, sizeof(BYTE), old, &old);
	}


    if (EzSetCurrentProcessIsCritical)
        {
		cout << "\n[+] Hooked SetCurrentProcessIsCritical";
		VirtualProtect(EzSetCurrentProcessIsCritical, sizeof(BYTE), PAGE_EXECUTE_READWRITE, &old);
		memcpy(EzSetCurrentProcessIsCritical, patch, sizeof(patch));
		VirtualProtect(EzSetCurrentProcessIsCritical, sizeof(BYTE), old, &old);
	}

    if (EzSetInformationThread)
		{
        cout << "\n[+] Hooked NtSetInformationThread";
        VirtualProtect(EzSetInformationThread, sizeof(BYTE), PAGE_EXECUTE_READWRITE, &old);
        memcpy(EzSetInformationThread, patch, sizeof(patch));
        VirtualProtect(EzSetInformationThread, sizeof(BYTE), old, &old);
        }

    if (EzSetInformationFile)
        {
		cout << "\n[+] Hooked NtSetInformationFile";
		VirtualProtect(EzSetInformationFile, sizeof(BYTE), PAGE_EXECUTE_READWRITE, &old);
		memcpy(EzSetInformationFile, patch, sizeof(patch));
		VirtualProtect(EzSetInformationFile, sizeof(BYTE), old, &old);
		}
